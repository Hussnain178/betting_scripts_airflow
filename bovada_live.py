...
class BovadaLiveOddsSpider(scrapy.Spider):
    ...
    def __init__(self):
        ...
        # Initialize logger
        self.custom_logger = setup_scraper_logger('bovada_live')
        log_scraper_progress(self.custom_logger, 'INIT', 'Initializing Bovada Live scraper')
    ...
    def start_requests(self):
        log_scraper_progress(self.custom_logger, 'START', 'Starting Bovada Live navigation scraping')
    ...
    def parse_navigation(self, response, **kwargs):
        try:
            ...
            log_scraper_progress(
                ...
                f'Found {len(all_category_data)} sport categories for live matches'
            )
            ...
            log_scraper_progress(
                ...
                f'Processing {live_sport_count} valid sports for live matches'
            )
    ...
    def parse_subcategories(self, response):
        try:
            ...
            log_scraper_progress(
                ...
                f'Found {len(subcategory_data)} subcategories for live {self.current_sport_name}'
            )
            ...
            log_scraper_progress(
                ...
                f'Failed to parse live subcategories for {self.current_sport_name}',
                error=parsing_error
            )

    def _process_hierarchy_node(self, hierarchy_node):
        """Process hierarchy nodes recursively to find leaf matches - Live version"""
        if 'leaf' not in hierarchy_node:
            return

        if hierarchy_node['leaf']:
            # This is a leaf node - get LIVE matches
            if 'link' in hierarchy_node:
                try:
                    ...
                    # IMPORTANT: Use liveOnly=true for live matches
                    live_matches_url = (
                        f'https://www.bovada.lv/services/sports/event/coupon/events/A/description/'
                        f'{league_hierarchy_data[-1]}?marketFilterId=def&liveOnly=true&eventsLimit=5000&lang=en'
                    )
                    ...
                    yield scrapy.Request(
                        ...
                        callback=self.extract_live_match_data,
                        meta={
                    ...
                    log_scraper_progress(
                        self.custom_logger, 'LIVE_HIERARCHY_ERROR',
                        f'Error processing live leaf: {hierarchy_error}',
                        error=hierarchy_error
                    )
        else:
            # This is not a leaf node - need to go deeper for live matches
            deeper_url = (
                f'https://services.bovada.lv/services/sports/event/v2/nav/A/description'
    ...
    def parse_deeper_hierarchy(self, response):
        """Parse deeper hierarchy levels for live matches"""
        try:
                    ...
                    try:
                        if node['leaf']:
                            if 'link' in node:
                                ...
                                live_matches_url = (
                                    f'https://www.bovada.lv/services/sports/event/coupon/events/A/description/'
                                    f'{league_name_hierarchy[-1]}?marketFilterId=def&liveOnly=true&eventsLimit=5000&lang=en'
                                )
                        ...
                        log_scraper_progress(
                            self.custom_logger, 'DEEP_LIVE_HIERARCHY_ERROR',
                            f'Error processing live hierarchy node: {node_error}',
                            error=node_error
                        )
                        continue
            ...
            log_scraper_progress(
                self.custom_logger, 'DEEP_LIVE_HIERARCHY_JSON_ERROR',
                'JSON decode error in deeper live hierarchy',
                error=json_error
            )
        except Exception as general_error:
            log_scraper_progress(
                self.custom_logger, 'DEEP_LIVE_HIERARCHY_GENERAL_ERROR',
                'General error in deeper live hierarchy parsing',
                error=general_error
            )

    def extract_live_match_data(self, response):
        try:
            match_data_response = json.loads(response.text)

            if not match_data_response or not match_data_response[0].get('events'):
                log_scraper_progress(self.custom_logger, 'NO_LIVE_MATCH_DATA', 'No live match data found in response')
                return

            live_matches_in_league = match_data_response[0]['events']
            log_scraper_progress(
                self.custom_logger, 'LIVE_MATCH_DATA_EXTRACTED',
                f'Processing {len(live_matches_in_league)} LIVE matches from league'
            )
            ...
            log_scraper_progress(
                self.custom_logger, 'LIVE_MATCH_EXTRACTION_ERROR',
                'Error extracting live match data',
                error=extraction_error
            )

    def _process_single_live_match(self, live_match_info, league_data):
        ...
        try:
            ...
            if live_match_info['awayTeamFirst']:
                competitor1_name = live_match_info['competitors'][1]['name']
                competitor2_name = live_match_info['competitors'][0]['name']
            else:
    ...
    def _extract_odds_from_live_match(self, live_match_info, match_data, desc_team1, desc_team2, short_team1,
                                      short_team2):
        """Extract odds information from LIVE match display groups"""
        for display_group in live_match_info['displayGroups']:
            ...
            for market_info in display_group['markets']:
                ...
                # Process market name for live matches
                market_name = self._process_live_market_name(
                ...
                if 'o/u' in final_market_name.lower() and '-' in final_market_name.lower():
                    continue
                ...
                self._process_live_market_outcomes(
    ...
    def _process_live_market_name(self, market_info, match_data, desc_team1, desc_team2, short_team1, short_team2):
        ...
        if 'period' in market_info:
            if market_info['period']['description'] == 'Live Regulation Time':
                # Keep the name as is for Live Regulation Time
    ...
    def _process_live_market_outcomes(self, market_info, match_data, header_category, market_name,
        ...
        if len(outcomes_list) == 2:
            self._process_two_outcome_live_market(
                outcomes_list, match_data, header_category, market_name,
                value_mappings, desc_team1, desc_team2, short_team1, short_team2
            )
        elif len(outcomes_list) == 3:
            self._process_three_outcome_live_market(
                outcomes_list, match_data, header_category, market_name,
                value_mappings, desc_team1, desc_team2, short_team1, short_team2
            )
        else:
            self._process_multiple_outcome_live_market(
                outcomes_list, market_info, match_data, header_category, market_name,
                value_mappings, desc_team1, desc_team2, short_team1, short_team2
            )

    def _process_two_outcome_live_market(self, outcomes, match_data, header_category, market_name,
                                         value_mappings, desc_team1, desc_team2, short_team1, short_team2):
        """Process live market with exactly 2 outcomes"""
    ...
    def _process_three_outcome_live_market(self, outcomes, match_data, header_category, market_name,
                                           value_mappings, desc_team1, desc_team2, short_team1, short_team2):
        """Process live market with exactly 3 outcomes"""
        # Get handicap value
        handicap_value = self._extract_handicap_value(outcomes[0])
    ...
    def _process_multiple_outcome_live_market(self, outcomes, market_info, match_data, header_category,
        ...
        """Process live market with multiple outcomes"""
        for individual_outcome in outcomes:
            ...
            # Handle period-specific outcomes for live matches
    ...
    def _match_live_data_with_flashscore(self, bovada_live_match_info):
        ...
        #             bovada_live_match_info['competitor1'].lower(),
        #             bovada_live_match_info['competitor2'].lower()
        #         )
        ...
        #         {"$set": {
        #             "prices.bovada": bovada_live_match_info['odds'],
    ...
    def close(self, reason):
        ...
        try:
            ...
            log_scraper_progress(
                ...
                f'Bovada Live scraper completed successfully',
...
if __name__ == '__main__':
    ...
    crawler_process.crawl(BovadaLiveOddsSpider)
